    ...
    try {
      ...
      if (+raqam && splited[0] > 0 && splited[0] < 115) {
        let response = await fetch(
          `https://api.quran.sutanlab.id/surah/${soz}`
        );
        let res = await fetch(
          `https://quranenc.com/api/translation/sura/uzbek_mansour/${splited[0]}`
        );
        response = await response.json();
        res = await res.json();
        bot.sendAudio(chatId, `${response.data.audio.primary}`, {
          caption: `${splited[0]}. ${
            response.data.surah.name.transliteration.en
          }\n${response.data.text.arab}\n${
            res.result[splited[1] - 1].translation
          }`,
          title: `${response.data.surah.name.transliteration.en}`,
        });
      }
    } catch (err) {
      bot.sendMessage(chatId, "Noto'g'ri formatda kiritdingiz !");
